...
namespace Gemserk.ECS
{
	...
	public struct ComponentTuple<T1, T2, T3, T4>
		where T1 : struct, IComponent 
		where T2 : struct, IComponent
		where T3 : struct, IComponent
		...
		ComponentArray<T2> componentArray2;
		ComponentArray<T3> componentArray3;
		ComponentArray<T4> componentArray4;
		...
		public T4 component4 {
			get {
				return componentArray4[_entityIndex];
			} 
			set {
		...
		{
			_entityIndex = 0;
			group = entityManager.GetComponentGroup(typeof(T1), typeof(T2), typeof(T3), typeof(T4));
			componentArray1 = group.GetComponent<T1>();
			componentArray2 = group.GetComponent<T2>();
			componentArray3 = group.GetComponent<T3>();
			componentArray4 = group.GetComponent<T4>();
		}
